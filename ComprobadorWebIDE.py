...
def GrabarcionInicialXML():
	...
	buscar4 = ET.SubElement(comprobar, 'buscar')
	buscar4.set('web','http://cdimage.deepin.com/releases/')
	buscar4.set('valor','15.12')
	buscar4.set('texto','Deepin 15.12')
...
def GrabarXML():
	if len(mc) != 0:
		...
		mydata = ET.tostring(comprobar)
		myfile = open("ComprobadorWeb.xml", "w")
		myfile.write(mydata)

def CargarXML():
	print("Cargar xml...")
	xmldoc = minidom.parse("ComprobadorWeb.xml")
...
def onEliminarClicked(builder):
	if len(mc) != 0:
		...
		if iter is not None:
			print("%s se ha eliminado" % (model[iter][0]))
			mc.remove(iter)
		else:
...
builder = Gtk.Builder()
builder.add_from_file("ComprobadorWeb.glade")
